...
class Converter {
    ...
    companion object {
        ...
        fun doubleArrayToPoints(array: DoubleArray): List<Point2D> {
            val result = arrayListOf<Point2D>()

            var i = 0
            while (i < array.size) {
                result.add(Point2D(array[i], array[i + 1]))
        ...
        fun pointsToDoubleArray(points: List<Point2D>): DoubleArray {
            val array = DoubleArray(points.size * 2)
            var i = 0
            points.forEach {
                array[i] = it.x
                array[i+1] = it.y
