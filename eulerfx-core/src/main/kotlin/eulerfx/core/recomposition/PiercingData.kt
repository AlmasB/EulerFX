...
class PiercingData(numZones: Int, private val cluster: List<Zone>, private val allZones: List<Zone>) {
    ...
    init {
        if (numZones == 4) {
            center = cluster.map { it.polygon.vertices() }
                    .flatten()
                    .groupBy { it.asInt }
    ...
    private fun computeRadius(potentialCenter: Point2D): Double {
        return allZones
                .minus(cluster)
                .map {
                    if (it.polygon is MultiPolygon2D) {

                        // check signed distance and also of the complement
                        val dist1 = Math.abs(it.polygon.boundary().signedDistance(potentialCenter.x, potentialCenter.y))
                        val dist2 = Math.abs(it.polygon.complement().boundary().signedDistance(potentialCenter.x, potentialCenter.y))

                        Math.min(dist1, dist2)
                    } else {
                        it.polygon.distance(potentialCenter.x, potentialCenter.y)
                    }
                }
                .sorted()
